...
Data-source favorites now translate during backfill to typed `source` references. Current catalog/data-source CRUD is not converted to outbox transactions in this slice because catalog audit writes and encrypted connection mutation must first share a reviewed unit of work; replay mappings are registered but must remain disabled. Shared-source visibility also differs from owner-isolated product reads, so live shadow parity is pending a visibility contract. PostgreSQL and the encrypted connection column remain authoritative.
