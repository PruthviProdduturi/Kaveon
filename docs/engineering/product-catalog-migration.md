...
KaveonDB accepts an owner-isolated immutable `query_history` record keyed by the source UUID, with an optional typed dataset reference. The canonical document contains the SQL Lab fields needed to reproduce history views and excludes additive Engine detail telemetry. A repeatable-read snapshot caps migration at 100,000 documents and exact owner reconciliation handles ambiguous create outcomes. `KAVEON_QUERY_HISTORY_OUTBOX_ENABLED=true` makes append plus its canonical event atomic and bounds owner deletion at 100 locked rows; it is off by default because the live outbox is absent. `KAVEON_QUERY_HISTORY_SHADOW_READ_ENABLED=true` compares at most 50 owner rows without changing PostgreSQL results. A product retention rule, checkpointed operator command, live replay/parity, fencing and rollback evidence remain pending; PostgreSQL stays authoritative.
