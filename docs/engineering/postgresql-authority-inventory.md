...
| `saved_queries` | `services/saved_queries.py` | `saved_query` product record | Default-off atomic outbox, deterministic checkpointed backfill and bounded owner shadow exist; live reconciliation and cutover remain |
...
| `context_snapshots`, `context_answer_cache` | `dlm/profiler.py`, context routes | Deliberately retired derived state; rebuild from the cutover dataset revision | Fence cache writes, record metadata-only source counts/schema identities, prove complete deterministic rebuild, then verify deletion; cached questions, answers, profiles, SQL and results must never enter retirement evidence |
