...
| `data_sources` | `routers/data_sources.py`, connection pool resolution, credentials service | Typed non-secret `source`; encrypted connection remains separate authority | Source mutation/outbox atomicity and favorite delete restriction exist; Key Vault resolution, rotation, visibility parity and live evidence remain |
