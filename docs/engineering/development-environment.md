...
## Historical environment baseline (before correctness and security fixes)

The observations in this section record the initial baseline, not the current
engine. The later 37-case SQL gate and five-worker same-file checks passed;
tracked API security tests and memory, cancellation, paging and worker-loss
qualification now exist. Consult the
[current readiness evidence](engine-readiness-qualification.md) for report paths
and remaining gates. The historical failures below are retained for comparison.
...
cloud qualification is claimed here; subsequent five-worker evidence is linked above.
...
The initialization script creates the ignored `.env.local` once, with random
Engine tokens and an explicit credential encryption keyring. Preserve this file
to decrypt locally stored connections; repeated initialization leaves it intact.
The Compose stack uses an explicit development transport boundary and loopback
published ports. It is not a production deployment configuration.

If Docker package downloads fail through the local network, the installed
environment has verified these HTTPS package feeds without disabling TLS checks:

```powershell
$env:KAVEON_NPM_REGISTRY = 'https://packagefeedproxy.microsoft.io/npm/'
$env:KAVEON_PIP_INDEX_URL = 'https://packagefeedproxy.microsoft.io/pypi/simple/'
