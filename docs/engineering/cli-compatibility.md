...
| `EXPLAIN` | Runs the statement with the result cache off and prints the query record's logical plan as an indented tree; no distributed fragment plan yet |
| `EXPLAIN ANALYZE` | The optimized plan, the coordinator's phase timings and, per stage, one row per task with node, elapsed, CPU, peak memory, rows and bytes scanned, rows out, exchange volumes and spill — the query record's counters, nothing estimated. Trino's per-operator statistics inside the plan are not mirrored |
