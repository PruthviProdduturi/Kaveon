...
## Re-measured on `bb82ea9` (2026-09-16, digest `791e8063…`)

After the correctness sweep (`docs/qualification/scale-suite/differential-2026-09-16.json`: 28 of 28 statement shapes value-identical across the dictionary-page and plain objects on AKS) and the distributed DISTINCT / COUNT(DISTINCT) work, the same twenty statements on the same object: **15 of 20 targets, faster than Trino on 9 of 13** (`scale-suite/2026-09-16-bb82ea9.json`). Nothing regressed beyond run-to-run noise; the four Trino wins are unchanged — `Y01` 3.17 vs 2.45, `Y05` 3.85 vs 2.72, `Y06` 3.99 vs 2.27, `C02` 6.00 vs 3.24 — and `sum_by_pair` sits on its 12 s target at 12.06. The worker budget for this run is the one every later record uses: 3 GiB per query, 4 GiB admission, inside the 6 GiB pod limit Trino's workers get.
