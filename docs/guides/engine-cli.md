...
| `ANALYZE [catalog.][schema.]table [WITH (…)]` | Coordinator statement (admin role): collects the table's row count, file count and bytes, each column's null fraction, minimum, maximum and data size, and every file's bounds from metadata alone, stored in the durable catalog beside the table and versioned by the source version. `WITH (sketches = true)` also reads every column once for distinct-count and quantile sketches; `WITH (distinct = true)` counts the distinct values of every column, `WITH (columns = ARRAY['a', 'b'])` of the columns named — exact, one `COUNT(DISTINCT)` statement per column through the cluster, cancellable with the `ANALYZE`; a column not counted keeps its previous count while the source version is unchanged. `WITH (cube = true)` builds the table's cube over its declared shape (`ALTER TABLE … SET SHAPE (…)`) in one scan. The result adds `distinct_columns`, how many were counted, and `cube_cells`, the cube's cells when one was built. |
