...
| `KAVEON_CONFIG_ENV_FALLBACK` | none | `false` | boolean | If the mounted config file is malformed, ignore it and load defaults plus environment settings. Requires `KAVEON_INSECURE_DEVELOPMENT=true` or `KAVEON_TLS_PROXY_BOUNDARY=true`; intended for legacy ACA mounts only. | both | set only during controlled ACA compatibility rollout |
| `KAVEON_SECURITY_JSON` | the whole `SecurityConfig` | none (no principals: nothing can submit) | JSON (TOML accepted only for legacy fallback) | `principals` (token, principal, role: `reader`, `analyst`, `admin`), `bridge_token`, `entra`, `studio_url`, `insecure_development`, and the legacy `resource_groups` list (name, principals with one optional `*` catch-all, `max_running`, `max_queued`, `queue_timeout_ms`), still accepted and translated into resource groups at start when nothing else configures them (see [Governance](governance.md)). Validated on start; tokens must be nonempty and distinct. A malformed value is fatal unless the explicit config environment fallback is enabled with insecure development; that emergency path discards the malformed document and must not be used for production authentication. | coordinator (statements), worker (its own credentials) | from the `kaveon-engine-auth` secret, key `security.json` |
