...
| `KAVEON_STATISTICS_AUTO_REFRESH` | none | `true` | boolean | Whether the coordinator refreshes a table's statistics in the background when planning observes a source version newer than the one on record (files added to a full document folded in; a removal or a metadata-only document recomputed at the document's depth; one refresh per table at a time). Off, statistics change only through `ANALYZE`; a stale record still costs a plan and never answers a statement. | coordinator | not set (default) |
