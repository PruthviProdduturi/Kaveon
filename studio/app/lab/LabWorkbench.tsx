...
export interface LabWorkbenchProps {
  /** Rendered inside the Catalog shell: no sidebar of its own, source and schema come from the caller. */
  embedded?: boolean;
...
export function LabWorkbench({ embedded = false, engineSourceId: embeddedSourceId = null, schema: embeddedSchema = null }: LabWorkbenchProps) {
  ...
  useEffect(() => {
    ...
    // Reuse a tab that already holds this exact SQL; otherwise open one named for what it queries.
    const name = searchParams.get("name") || "Dataset Query";
    const newId = `url-${Date.now()}`;
    let reuse: string | null = null;
    setQueries((prev) => {
      const existing = prev.find((tab) => tab.text === sql);
      if (existing) { reuse = existing.id; return prev; }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(saved) && saved.length > 0) {
        // Merge, never replace: a tab opened from a URL, a dataset page or the
        // history page may already exist by the time persisted tabs arrive.
        let keepActive = false;
        setQueries((prev) => {
          const fresh = prev.filter((tab) => tab.id !== "q1" && !saved.some((persisted) => persisted.id === tab.id));
          keepActive = fresh.length > 0;
          return [...saved, ...fresh];
        });
        setActiveQueryId((current) => (keepActive ? current : savedActive || saved[0].id));
      }
    } catch { /* ignore */ }
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [embedded, isLoadingDatabases, isLoadingEngineSources, currentDataSourceId, currentEngineSourceId, engineSources, dataSources]);
  ...
  const loadEngineSchemas = async (sourceId: string) => {
    ...
    try {
      ...
      const schema = (embeddedSchemaRef.current && schemas.find((name) => name === embeddedSchemaRef.current))
        || schemas.find((name) => name.toLowerCase() === "nyc_taxi")
...
return;
  ...
  return (
    <>
      ...
      {isAuthenticated && (
        <div className="main-content" style={embedded ? { height: "calc(100vh - 84px)" } : undefined}>
          {/* Sidebar */}
          {!embedded && (
