"""Checkpointed, default-dry user theme backfill operation."""
...
def _document(snapshot, next_index, complete):
    value = {"version": VERSION, "family": "user_themes", "source_watermark": snapshot.source_watermark,
...
def save(path, snapshot, next_index, complete=False):
    ...
    if not 0 <= next_index <= len(snapshot.records) or (complete and next_index != len(snapshot.records)):
        raise RuntimeError("user theme checkpoint position is invalid")
    encoded = json.dumps(_document(snapshot, next_index, complete), sort_keys=True,
                         separators=(",", ":")).encode()
    if len(encoded) > MAX_CHECKPOINT_BYTES:
        raise RuntimeError("user theme checkpoint exceeds its byte bound")
...
def load(path):
    ...
    try:
        ...
        if claimed != hashlib.sha256(json.dumps(raw, sort_keys=True, separators=(",", ":")).encode()).hexdigest() \
                or raw["version"] != VERSION or raw["family"] != "user_themes":
            raise RuntimeError("user theme checkpoint identity is invalid")
        records = tuple(backfill.ThemeRecord(**record) for record in raw["records"])
        snapshot = backfill.ThemeSnapshot(int(raw["source_watermark"]),
        ...
        raise RuntimeError("user theme checkpoint is invalid") from error


def run(checkpoint, *, apply, resume):
    if apply and os.getenv("KAVEON_USER_THEME_MIGRATION_ENABLED") != "true":
        raise RuntimeError("Apply requires KAVEON_USER_THEME_MIGRATION_ENABLED=true")
    if resume:
    ...
    for index in range(position, len(snapshot.records)):
        ...
        single = backfill.ThemeSnapshot(snapshot.source_watermark, (record,),
