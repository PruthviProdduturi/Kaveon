"""Checkpointed, default-dry source backfill operation."""
...
from services import source_backfill as backfill

VERSION, MAX_CHECKPOINT_BYTES = 1, 8 * 1024 * 1024


def _document(snapshot, next_index, complete):
    value = {"version": VERSION, "family": "sources", "source_watermark": snapshot.source_watermark,
...
def save(path, snapshot, next_index, complete=False):
    ...
    if not 0 <= next_index <= len(snapshot.records) or (complete and next_index != len(snapshot.records)):
        raise RuntimeError("source checkpoint position is invalid")
    encoded = json.dumps(_document(snapshot, next_index, complete), sort_keys=True,
                         separators=(",", ":")).encode()
    if len(encoded) > MAX_CHECKPOINT_BYTES:
        raise RuntimeError("source checkpoint exceeds its byte bound")
...
def load(path):
    ...
    try:
        ...
        if claimed != hashlib.sha256(json.dumps(raw, sort_keys=True, separators=(",", ":")).encode()).hexdigest() \
                or raw["version"] != VERSION or raw["family"] != "sources":
            raise RuntimeError("source checkpoint identity is invalid")
        records = tuple(backfill.SourceRecord(**record) for record in raw["records"])
        snapshot = backfill.SourceSnapshot(int(raw["source_watermark"]),
        ...
        raise RuntimeError("source checkpoint is invalid") from error


def run(checkpoint, *, apply, resume):
    if apply and os.getenv("KAVEON_SOURCE_MIGRATION_ENABLED") != "true":
        raise RuntimeError("Apply requires KAVEON_SOURCE_MIGRATION_ENABLED=true")
    if resume:
    ...
    for index in range(position, len(snapshot.records)):
        ...
        single = backfill.SourceSnapshot(snapshot.source_watermark, (record,),
                                         backfill.digest((record,)))
