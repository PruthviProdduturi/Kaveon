"""Checkpointed, default-dry DLM run metadata backfill operation."""
...
def _document(snapshot, next_index, complete):
    value = {"version": VERSION, "family": "dlm_runs",
             "source_watermark": snapshot.source_watermark,
             "definition_snapshot_id": snapshot.definition_snapshot_id,
             "snapshot_sha256": snapshot.snapshot_sha256, "next_index": next_index,
             "complete": complete, "records": [record.__dict__ for record in snapshot.records]}
...
def save(path: Path, snapshot, next_index: int, complete=False):
    ...
    if not 0 <= next_index <= len(snapshot.records) or (complete and next_index != len(snapshot.records)):
        raise RuntimeError("DLM run checkpoint position is invalid")
    encoded = json.dumps(_document(snapshot, next_index, complete), sort_keys=True,
                         separators=(",", ":")).encode()
    if len(encoded) > MAX_CHECKPOINT_BYTES:
        raise RuntimeError("DLM run checkpoint exceeds its byte bound")
...
def load(path: Path):
    ...
    try:
        ...
        if claimed != actual or raw["version"] != VERSION or raw["family"] != "dlm_runs":
            raise RuntimeError("DLM run checkpoint identity is invalid")
        records = tuple(backfill.RunRecord(**record) for record in raw["records"])
        snapshot = backfill.RunSnapshot(int(raw["source_watermark"]),
                                        str(raw["definition_snapshot_id"]), records,
...
def run(checkpoint: Path, artifact_root: Path, *, apply: bool, resume: bool):
    ...
    if resume:
        ...
        snapshot, position, complete = backfill.capture_snapshot(artifact_root), 0, False
