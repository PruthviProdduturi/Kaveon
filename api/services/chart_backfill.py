...
def apply_and_reconcile(snapshot):
    ...
    for record in snapshot.records:
        target = product_store.migration_read("chart", record.record_id, record.owner_principal, "Admin")
        ...
        if target is not None and (type(revision) is not int or revision < 1):
            raise RuntimeError(f"KaveonDB chart {record.record_id} has an invalid revision")
        try:
            product_store.migration_transact([product_store.ProductMutation(
                "update" if target is not None else "create", "chart", record.record_id,
                record.document, revision)], record.owner_principal, "Admin")
        except HTTPException as error:
            resolved = product_store.migration_read("chart", record.record_id, record.owner_principal, "Admin")
    ...
    return {"family": "charts", "source_watermark": snapshot.source_watermark,
            ...
            "created": created, "already_present": already_present, "repaired": repaired,
