"""Tamper-evident checkpoint/resume for activity migration."""
import hashlib,json,os,tempfile
from pathlib import Path
from services import activity_backfill as backfill
VERSION=1;MAX_CHECKPOINT_BYTES=32*1024*1024
def _body(s,index,complete):return {"version":VERSION,"family":"activity","source_watermark":s.source_watermark,"snapshot_sha256":s.snapshot_sha256,"next_index":index,"complete":complete,"records":[r.__dict__ for r in s.records]}
def save(path,s,index,complete=False):
 backfill.validate(s)
 if not 0<=index<=len(s.records) or (complete and index!=len(s.records)):raise RuntimeError("activity checkpoint position is invalid")
 ...
 if len(encoded)>MAX_CHECKPOINT_BYTES:raise RuntimeError("activity checkpoint exceeds its bound")
...
def load(path):
 if path.stat().st_size>MAX_CHECKPOINT_BYTES:raise RuntimeError("activity checkpoint exceeds its bound")
 try:
  raw=json.loads(path.read_text());claimed=raw.pop("checkpoint_sha256")
  if claimed!=hashlib.sha256(json.dumps(raw,sort_keys=True,separators=(",",":")).encode()).hexdigest() or raw["version"]!=VERSION or raw["family"]!="activity":raise RuntimeError("activity checkpoint identity is invalid")
  ...
  if not 0<=index<=len(records) or (complete and index!=len(records)):raise RuntimeError("activity checkpoint position is invalid")
  return s,index,complete
 except (KeyError,TypeError,ValueError,json.JSONDecodeError) as error:raise RuntimeError("activity checkpoint is invalid") from error
def run(path,*,apply,resume):
 if apply and os.getenv("KAVEON_ACTIVITY_MIGRATION_ENABLED")!="true":raise RuntimeError("apply requires KAVEON_ACTIVITY_MIGRATION_ENABLED=true")
 if resume:
