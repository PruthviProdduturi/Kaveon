"""Checkpointed, default-dry favorite backfill operation."""
...
from services import favorite_backfill as backfill

VERSION, MAX_CHECKPOINT_BYTES = 1, 8 * 1024 * 1024


def _document(snapshot, next_index, complete):
    value = {"version": VERSION, "family": "favorites", "source_watermark": snapshot.source_watermark,
             "target_snapshot_id": snapshot.target_snapshot_id, "snapshot_sha256": snapshot.snapshot_sha256,
...
def save(path, snapshot, next_index, complete=False):
    ...
    if not 0 <= next_index <= len(snapshot.records) or (complete and next_index != len(snapshot.records)):
        raise RuntimeError("favorite checkpoint position is invalid")
    encoded = json.dumps(_document(snapshot, next_index, complete), sort_keys=True,
                         separators=(",", ":")).encode()
    if len(encoded) > MAX_CHECKPOINT_BYTES:
        raise RuntimeError("favorite checkpoint exceeds its byte bound")
...
def load(path):
    ...
    try:
        ...
        if claimed != hashlib.sha256(json.dumps(raw, sort_keys=True, separators=(",", ":")).encode()).hexdigest() \
                or raw["version"] != VERSION or raw["family"] != "favorites":
            raise RuntimeError("favorite checkpoint identity is invalid")
        records = tuple(backfill.FavoriteRecord(**record) for record in raw["records"])
        snapshot = backfill.FavoriteSnapshot(int(raw["source_watermark"]), str(raw["target_snapshot_id"]),
        ...
        raise RuntimeError("favorite checkpoint is invalid") from error


def run(checkpoint, *, apply, resume):
    if apply and os.getenv("KAVEON_FAVORITE_MIGRATION_ENABLED") != "true":
        raise RuntimeError("Apply requires KAVEON_FAVORITE_MIGRATION_ENABLED=true")
    if resume:
    ...
    for index in range(position, len(snapshot.records)):
        ...
        single = backfill.FavoriteSnapshot(snapshot.source_watermark, snapshot.target_snapshot_id, (record,),
                                        backfill.snapshot_digest((record,), snapshot.target_snapshot_id))
