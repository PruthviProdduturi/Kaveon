...
from services.user_recent_backfill import canonical, record_id
...
def _document(row):
    ...
    return {"user_email":str(row["user_email"]),"item_id":str(row["item_id"]),"label":row.get("label"),"href":row.get("href"),"type":row.get("type"),"created_at":created}
def _event(tx,operation,row,actor):
    owner=str(row["user_email"]);item=str(row["item_id"])
    product_outbox.enqueue(tx,family="user_recents",operation=operation,record_id=record_id(owner,item),payload={} if operation=="delete" else _document(row),actor=actor,owner=owner)
...
def remove_recent(user_email: str, item_id: str) -> None:
    with db.transaction() as tx:
        row=tx.query_one("SELECT id,user_email,item_id FROM user_recents WHERE user_email=@param0 AND item_id=@param1 FOR UPDATE",[user_email,item_id])
        if not row:return
...
def clear_recents(user_email: str, item_type: str | None = None) -> int:
    ...
    with db.transaction() as tx:
        ...
        rows=tx.query(f"SELECT id,user_email,item_id FROM user_recents WHERE {predicate} ORDER BY item_id LIMIT 21 FOR UPDATE",params)["rows"]
...
def remove_recent_all_users(item_id: str, item_type: str) -> None:
    ...
    with db.transaction() as tx:
        rows=tx.query("SELECT id,user_email,item_id FROM user_recents WHERE item_id=@param0 AND type=@param1 ORDER BY user_email LIMIT @param2 FOR UPDATE",[item_id,item_type,MAX_CROSS_OWNER_DELETE+1])["rows"]
