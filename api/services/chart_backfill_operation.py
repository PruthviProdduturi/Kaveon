"""Checkpointed, default-dry chart backfill operation."""
...
def _document(snapshot, next_index, complete):
    value = {"version": VERSION, "family": "charts", "source_watermark": snapshot.source_watermark,
             "dataset_snapshot_id": snapshot.dataset_snapshot_id, "snapshot_sha256": snapshot.snapshot_sha256,
...
def save(path, snapshot, next_index, complete=False):
    ...
    if not 0 <= next_index <= len(snapshot.records) or (complete and next_index != len(snapshot.records)):
        raise RuntimeError("chart checkpoint position is invalid")
    encoded = json.dumps(_document(snapshot, next_index, complete), sort_keys=True,
                         separators=(",", ":")).encode()
    if len(encoded) > MAX_CHECKPOINT_BYTES:
        raise RuntimeError("chart checkpoint exceeds its byte bound")
...
def load(path):
    ...
    try:
        ...
        if claimed != hashlib.sha256(json.dumps(raw, sort_keys=True, separators=(",", ":")).encode()).hexdigest() \
                or raw["version"] != VERSION or raw["family"] != "charts":
            raise RuntimeError("chart checkpoint identity is invalid")
        records = tuple(backfill.ChartRecord(**record) for record in raw["records"])
        snapshot = backfill.ChartSnapshot(int(raw["source_watermark"]), str(raw["dataset_snapshot_id"]),
        ...
        raise RuntimeError("chart checkpoint is invalid") from error


def run(checkpoint, *, apply, resume):
    if apply and os.getenv("KAVEON_CHART_MIGRATION_ENABLED") != "true":
        raise RuntimeError("Apply requires KAVEON_CHART_MIGRATION_ENABLED=true")
    if resume:
    ...
    for index in range(position, len(snapshot.records)):
        ...
        single = backfill.ChartSnapshot(snapshot.source_watermark, snapshot.dataset_snapshot_id, (record,),
