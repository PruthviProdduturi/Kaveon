...
Authentication:
  • Browser → lens-web: Azure AD MSAL (PKCE, no client_secret)
  • Browser → lens-api: Azure AD Bearer tokens (RS256 verified by API)
  • lens-api → Fabric SQL: DefaultAzureCredential (Managed Identity token)
...
|---|---|---|
| `lens-web` | External (HTTPS) | Next.js frontend, served to browsers |
| `lens-api` | External (HTTPS) | FastAPI backend — REST API, business logic, and direct ODBC connection pool to Fabric SQL |

> The former `lens-python-proxy` Flask sidecar has been eliminated. The FastAPI backend handles ODBC connectivity in-process via `pyodbc` + `azure-identity`, removing the inter-service HTTP hop.
...
RG="lens-rg"              # your resource group name
...
```bash
ACR_NAME="lensacr"        # globally unique; lowercase alphanumeric only
...
3. Search for `lens-identity` (the display name of your UAMI)
...
1. **Authentication** tab:
   ...
   - Add `http://localhost:3000` and `https://<your-lens-web-fqdn>` as allowed redirect URIs
...
> After deploying lens-web, come back and add its FQDN as a redirect URI (e.g., `https://lens-web.politebeach-abc123.eastus.azurecontainerapps.io`).

### 6.4 Configure App Roles

Lens uses Azure AD App Roles to assign user permissions. Add these four roles to the App Registration manifest.
...
|---|---|---|---|
| Lens Viewer | `Lens.Viewer` | Users/Groups | Read-only access to published dashboards |
| Lens Analyst | `Lens.Analyst` | Users/Groups | SQL Lab, chart and dataset creation |
| Lens Editor | `Lens.Editor` | Users/Groups | All Analyst permissions + publish content |
| Lens Admin | `Lens.Admin` | Users/Groups | Full access including user and data source management |

3. Assign users: **Enterprise Applications** → **[your Lens app]** → **Users and groups** → **Add user/group** → select user → select role
...
4. Fill in:
   ...
   - **Repository**: your repository name (e.g., `my-org/Lens`)
   - **Entity type**: **Branch**
   - **Branch**: `main`
   - **Name**: `lens-main-deploy`
5. Click **Add**
...
GITHUB_SP_ID=$(az ad sp list \
  --display-name "Lens GitHub Actions" \
  --query "[0].id" -o tsv)
...
# Build and push lens-api image
docker build -t "${ACR_NAME}.azurecr.io/lens-api:init" \
  -f apps/lens-api/Dockerfile .
docker push "${ACR_NAME}.azurecr.io/lens-api:init"

# Build and push lens-web image (placeholder build-args)
docker build \
  ...
  -t "${ACR_NAME}.azurecr.io/lens-web:init" \
  -f apps/lens-web/Dockerfile .
docker push "${ACR_NAME}.azurecr.io/lens-web:init"
```

### 8.1 Deploy lens-api

```bash
API_APP_NAME="lens-api"
...
az containerapp create \
  ...
  --image "${ACR_NAME}.azurecr.io/lens-api:init" \
...
# e.g. lens-api.politebeach-abc123.eastus.azurecontainerapps.io
```

Update lens-api with its own public URL for CORS:
```bash
az containerapp update \
  --name "$API_APP_NAME" \
  --resource-group "$RG" \
  --set-env-vars "WEB_URL=https://<lens-web-fqdn>"
```
...
docker build \
  ...
  -t "${ACR_NAME}.azurecr.io/lens-web:init-configured" \
  -f apps/lens-web/Dockerfile .
docker push "${ACR_NAME}.azurecr.io/lens-web:init-configured"

az containerapp create \
  --name "$WEB_APP_NAME" \
  --resource-group "$RG" \
  --environment "$CAE_NAME" \
  --image "${ACR_NAME}.azurecr.io/lens-web:init-configured" \
  --registry-server "${ACR_NAME}.azurecr.io" \
  --registry-identity "$IDENTITY_RESOURCE_ID" \
  --user-assigned-identities "$IDENTITY_RESOURCE_ID" \
  --ingress external \
...
# e.g. lens-web.politebeach-abc123.eastus.azurecontainerapps.io
```
...
2. Update lens-api's `WEB_URL` env var to `https://${WEB_FQDN}` (for CORS)
3. Rebuild and redeploy lens-web with `WEB_URL=https://${WEB_FQDN}` as the `--build-arg`
...
| `ACR_NAME` | `lensacr` | Your ACR name (without `.azurecr.io`) |
| `RESOURCE_GROUP` | `lens-rg` | Azure resource group |
| `LENS_API_FQDN` | `lens-api.politebeach-abc123.eastus.azurecontainerapps.io` | API FQDN **without** `https://` |
| `LENS_WEB_URL` | `https://lens-web.politebeach-abc123.eastus.azurecontainerapps.io` | Web URL **with** `https://` |
...
| `CONTAINER_APP_WEB` | `lens-web` | Container App name |
| `CONTAINER_APP_API` | `lens-api` | Container App name |
...
| `WEB_URL` | Container App env var | Allowed CORS origin (your lens-web FQDN with `https://`) |
...
|---|---|---|
| `API_URL` | `NEXT_PUBLIC_API_BASE_URL`, `NEXT_PUBLIC_API_URL` | Full `https://` URL of lens-api |
...
On first access, Lens detects that the metadata database schema has not been initialised and presents the **Setup Wizard**.
...
Before switching to Azure AD auth, assign the **Admin** App Role to yourself in Azure portal (Enterprise Applications → Lens → Users and groups). Then save the auth config — you will be signed in as Admin on the next login.

### Triggering the wizard

Navigate to `https://<your-lens-web-fqdn>`. If `FABRIC_METADATA_ENDPOINT` / `FABRIC_METADATA_DATABASE` are not set, the wizard modal appears automatically after sign-in.
...
git push origin main
    └─► GitHub Actions trigger
            ├─ docker build + push: lens-api:<sha>
            ├─ docker build + push: lens-web:<sha>  (bakes API_URL, AAD creds)
            ├─ az containerapp update: lens-api
            └─ az containerapp update: lens-web
```
...
These are baked at **build time**, not runtime. Update the GitHub variable (`LENS_API_FQDN`, `AZURE_CLIENT_ID`, etc.) and **re-run the workflow** to rebuild the image with the correct values.
...
2. Check their assignment: **Enterprise Applications → [Lens app] → Users and groups**
3. Or check the DB-level assignment at `GET /api/v1/users` (Admin only)

### First page load is slow (~10–12 seconds)

This is expected on the **very first sign-in** after a deployment or after a period of inactivity. Microsoft Fabric SQL Serverless has a cold-start time of approximately 10 seconds when no connections have been made recently. Lens mitigates this with:
