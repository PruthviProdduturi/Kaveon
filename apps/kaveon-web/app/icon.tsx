...
function apertureSvg(px: number): string {
  ...
  const lines = L.map(b =>
    `<line x1="${b.ix.toFixed(2)}" y1="${b.iy.toFixed(2)}" x2="${b.ox.toFixed(2)}" y2="${b.oy.toFixed(2)}" stroke="#d4a017" stroke-width="1.6" stroke-linecap="round"/>`
  ).join('')
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${px}" height="${px}" viewBox="0 0 24 24" fill="none">` +
    `<circle cx="12" cy="12" r="10.6" stroke="#d4a017" stroke-width="1.5" fill="none"/>` +
