...
export default function AboutPage() {
  ...
  return (
    <div style={{ background: "#0a0a0a", color: "#f5f5f5", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      {/* ─── Architecture ─── */}
      <Section style={{ maxWidth: 1000, margin: "0 auto", padding: "0 24px 120px" }}>
        <h2 style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "4px", color: "#4A9EE8", textAlign: "center", marginBottom: 48 }}>
          Architecture
        </h2>
        <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", gap: 0 }}>
          {/* Browser */}
          <div style={{ width: "100%", maxWidth: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "28px 0" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "20px 36px", borderRadius: 14, background: "linear-gradient(135deg, rgba(74,158,232,0.08), rgba(74,158,232,0.02))", border: "1px solid rgba(74,158,232,0.15)" }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#4A9EE8" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
              <div>
                <div style={{ fontSize: 15, fontWeight: 600, color: "#e2e8f0" }}>Browser</div>
                <div style={{ fontSize: 11, color: "#666" }}>You · Your team</div>
              </div>
            </div>
          </div>
          {/* Arrow */}
          <div style={{ width: 2, height: 32, background: "linear-gradient(180deg, rgba(74,158,232,0.4), rgba(74,158,232,0.1))" }} />
          <svg width="12" height="8" viewBox="0 0 12 8" style={{ marginTop: -1 }}><path d="M6 8L0 0h12z" fill="rgba(74,158,232,0.3)" /></svg>

          {/* Frontend + Backend row */}
          <div style={{ width: "100%", maxWidth: 700, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, padding: "20px 0" }}>
            {/* Frontend */}
            <div style={{ padding: "24px", borderRadius: 14, background: "linear-gradient(135deg, rgba(16,185,129,0.06), rgba(16,185,129,0.01))", border: "1px solid rgba(16,185,129,0.12)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="1.5"><polygon points="12 2 22 8.5 22 15.5 12 22 2 15.5 2 8.5 12 2"/><line x1="12" y1="22" x2="12" y2="15.5"/><polyline points="22 8.5 12 15.5 2 8.5"/></svg>
                <span style={{ fontSize: 14, fontWeight: 600, color: "#e2e8f0" }}>kaveon-web</span>
              </div>
              <div style={{ fontSize: 12, color: "#777", lineHeight: 1.6 }}>
                Next.js 15 · React 19 · TypeScript<br/>
                ECharts · Monaco · NextAuth<br/>
                NL→SQL engine · Chat UI
              </div>
              <div style={{ marginTop: 10, fontSize: 11, color: "#10b981", display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#10b981" }} />
                Vercel
              </div>
            </div>
            {/* Backend */}
            <div style={{ padding: "24px", borderRadius: 14, background: "linear-gradient(135deg, rgba(139,92,246,0.06), rgba(139,92,246,0.01))", border: "1px solid rgba(139,92,246,0.12)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" strokeWidth="1.5"><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/><line x1="6" y1="6" x2="6.01" y2="6"/><line x1="6" y1="18" x2="6.01" y2="18"/></svg>
                <span style={{ fontSize: 14, fontWeight: 600, color: "#e2e8f0" }}>kaveon-api</span>
              </div>
              <div style={{ fontSize: 12, color: "#777", lineHeight: 1.6 }}>
                FastAPI · Python 3.11<br/>
                SQL execution · RBAC<br/>
                AI service · Connection pools
              </div>
              <div style={{ marginTop: 10, fontSize: 11, color: "#8b5cf6", display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#8b5cf6" }} />
                Azure Container Apps
              </div>
            </div>
          </div>
          {/* Connection line between frontend and backend */}
          <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, 20px)", width: 2, height: 40, display: "none" }} />

          {/* Arrow down */}
          <div style={{ width: 2, height: 32, background: "linear-gradient(180deg, rgba(139,92,246,0.4), rgba(245,158,11,0.1))" }} />
          <svg width="12" height="8" viewBox="0 0 12 8" style={{ marginTop: -1 }}><path d="M6 8L0 0h12z" fill="rgba(245,158,11,0.3)" /></svg>

          {/* Data Sources */}
          <div style={{ width: "100%", maxWidth: 700, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, padding: "20px 0" }}>
            {[
              { name: "PostgreSQL", color: "#4A9EE8", sub: "Neon · Azure" },
              { name: "Fabric SQL", color: "#f59e0b", sub: "Microsoft" },
              { name: "Azure SQL", color: "#06b6d4", sub: "Managed" },
              { name: "MySQL", color: "#ec4899", sub: "MariaDB" },
            ].map(db => (
              <div key={db.name} style={{ padding: "16px", borderRadius: 10, background: `linear-gradient(135deg, ${db.color}10, ${db.color}03)`, border: `1px solid ${db.color}20`, textAlign: "center" }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0", marginBottom: 4 }}>{db.name}</div>
                <div style={{ fontSize: 10, color: "#666" }}>{db.sub}</div>
              </div>
            ))}
          </div>

          {/* Proxy label */}
          <div style={{ position: "absolute", top: "38%", left: "50%", transform: "translateX(-50%)", fontSize: 10, color: "#555", background: "#0a0a0a", padding: "2px 10px", borderRadius: 4 }}>
            /api/kaveon/* proxy
          </div>
        </div>
      </Section>
