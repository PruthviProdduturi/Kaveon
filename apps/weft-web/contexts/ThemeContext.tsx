...
export function ThemeProvider({ children }: ThemeProviderProps) {
  ...
  const [primaryColor, setPrimaryColor] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('weft-theme-color');
      // Migrate old gray default to new Microsoft blue default
      if (saved && saved.toLowerCase() === '#8f9192') {
        localStorage.setItem('weft-theme-color', DEFAULT_THEME_COLOR);
  ...
  useEffect(() => {
    if (isAuthenticated && account?.email) {
      const cached = typeof window !== 'undefined'
        ? localStorage.getItem('weft-theme-color')
        : null;
  ...
  useEffect(() => {
    ...
    // Set both --theme-* and --weft-* variables for compatibility
    ...
    // Update Weft brand colors to match user's theme
    document.documentElement.style.setProperty('--weft-primary', primaryColor);
    document.documentElement.style.setProperty('--weft-secondary', gradients.light);
    document.documentElement.style.setProperty('--weft-accent', gradients.lighter);
    document.documentElement.style.setProperty('--weft-dark', gradients.dark);
    document.documentElement.style.setProperty('--weft-light', gradients.lighter);

    // Keep RGB component vars in sync so rgba(var(--weft-primary-rgb), α) works everywhere
    const toRgbStr = (hex: string) => { const { r, g, b } = hexToRGB(hex); return `${r}, ${g}, ${b}`; };
    document.documentElement.style.setProperty('--weft-primary-rgb',   toRgbStr(primaryColor));
    document.documentElement.style.setProperty('--weft-secondary-rgb', toRgbStr(gradients.light));
    document.documentElement.style.setProperty('--weft-accent-rgb',    toRgbStr(gradients.lighter));
    document.documentElement.style.setProperty('--weft-dark-rgb',      toRgbStr(gradients.dark));
  ...
  const loadUserTheme = async () => {
    try {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('weft-theme-color', colorToUse);
      }
    } catch (error) {
  ...
  const setTheme = async (color: string) => {
    try {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('weft-theme-color', color);
      }
