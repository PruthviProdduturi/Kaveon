...
/**
 * WeftLoading — full-screen branded loading screen.
 *
 * Colors are driven entirely by CSS custom properties (--weft-primary, etc.)
...
interface WeftLoadingProps {
  message?: string;
  fullScreen?: boolean;
}

export function WeftLoading({
  message = 'Loading',
  fullScreen = true
}: WeftLoadingProps) {
  ...
  const primary   = 'var(--weft-primary)';
  const secondary = 'var(--weft-secondary)';
  const accent    = 'var(--weft-accent)';
  const dark      = 'var(--weft-dark)';

  // rgba() helpers using CSS var RGB components (e.g. --weft-primary-rgb: "0, 120, 212")
  const pr = (opacity: number) => `rgba(var(--weft-primary-rgb), ${opacity})`;
  const sr = (opacity: number) => `rgba(var(--weft-secondary-rgb), ${opacity})`;
  const ar = (opacity: number) => `rgba(var(--weft-accent-rgb), ${opacity})`;
  const dr = (opacity: number) => `rgba(var(--weft-dark-rgb), ${opacity})`;
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '60px', width: '100%', maxWidth: '500px', padding: '20px' }}>
        ...
        <div style={{ position: 'relative', width: '320px', height: '320px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <div style={{
            ...
            <WeftLogo size={120} animate="pulse" />
          </div>
