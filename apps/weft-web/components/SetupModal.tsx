 ...
 * Setup Modal
 *
 * Guides the user through connecting a metadata database to Weft.
...
const DB_TYPES: Record<DbType, DbTypeConfig> = {
  ...
  azure_sql: {
    ...
    dbHint: "The Azure SQL database that will store Weft metadata.",
  },
  postgresql: {
    label: "PostgreSQL",
    defaultPort: 5432,
...
export function SetupModal({ data, onComplete }: SetupModalProps) {
            ...
            {cfg.usesConnectionString && (
              <>
                ...
                <p style={S.hint}>
                  ...
                  Weft extracts the server and database automatically.
                </p>
              </>
            )}
            ...
            {testOk ? (
              <>
                <div style={{
                  ...
                  <span>If this database already has Weft data, <strong>nothing will be deleted</strong> — tables are only created if they don&apos;t already exist.</span>
                </div>
  ...
  return (
    <div style={S.overlay}>
      <div style={S.card}>
        <div style={S.logoRow}><WeftLogo size={48} animate="pulse" /></div>
        ...
        {phase === "connection_failed" && (
          <ErrorState errType="connection_failed" title="Cannot Connect"
            description="Weft cannot reach the configured metadata database. Check your connection details and retry." />
        )}

        {phase === "access_denied" && (
          <ErrorState errType="access_denied" title="Access Denied"
            description="Weft reached the host but authentication failed. Verify credentials or Azure AD role assignments." />
        )}
        ...
        {phase === "schema_missing" && (
          <>
            ...
            <p style={S.sub}>
              Connected successfully. Click below to create the Weft tables in this database.
            </p>
            <div style={{
              background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.2)",
              ...
              <i className="fas fa-shield-check" style={{ marginTop: 1, flexShrink: 0 }} />
              <span>
                If this database already has Weft data, <strong>nothing will be deleted</strong> — tables are only created if they don&apos;t already exist.
              </span>
            </div>
            <div style={S.infoBox}>
        ...
        {phase === "initializing" && (
          <>
            ...
            <h2 style={S.heading}>Setting Up Weft…</h2>
        ...
        {phase === "success" && (
          <>
            ...
            <p style={S.sub}>
              Metadata database initialised. Weft is restarting — the page will reload automatically.
            </p>
