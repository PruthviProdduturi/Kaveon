 ...
 * For local auth (weft_auth_provider === "local"), reads the Bearer token from
...
export async function msalFetch(input: RequestInfo, init: RequestInit = {}): Promise<Response> {
  ...
  const storedProvider = typeof window !== "undefined" ? window.localStorage.getItem("weft_auth_provider") : null;
  if (!storedProvider || storedProvider === "local") {
    // Local auth: use stored JWT directly, no MSAL involved
    const localToken = window.localStorage.getItem("weft_local_token");
    if (localToken) {
