...
export default function LabPage() {
  ...
  useEffect(() => {
    ...
    const userEmail = account?.email || account?.username || null;
    const key = `weft_lab_tables_v1_${userEmail || 'anon'}`;
    try {
  ...
  useEffect(() => {
    if (!isAuthenticated || typeof window === 'undefined') return;
    const key = `weft_lab_tabs_v1_${account?.email || account?.username || 'anon'}`;
    try {
      const raw = localStorage.getItem(key);
      if (!raw) return;
    ...
    <>
      {!isAuthenticated && (
        <p className="muted" style={{ margin: "1.5rem" }}>
          Sign in and connect to Weft to use SQL Lab.
        </p>
      )}
                      ...
                      <div style={{ position: "relative" }}>
                        <button type="button" className="template-btn" style={{ padding: "0.25rem 0.6rem", fontSize: "0.75rem" }} onClick={() => setIsColumnPickerOpen((p) => !p)} title="Show/hide columns">
                          ...
                          {hiddenColumns.size > 0 && (
                            <span style={{ marginLeft: 4, background: "var(--weft-primary,#0078D4)", color: "#fff", borderRadius: 9, padding: "0 5px", fontSize: "0.7rem" }}>{hiddenColumns.size}</span>
                          )}
                        </button>
                        {isColumnPickerOpen && (
                          <>
                            ...
                            <div style={{ position: "absolute", top: "110%", right: 0, zIndex: 200, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, boxShadow: "0 4px 12px rgba(15,23,42,0.12)", minWidth: 180, maxHeight: 280, overflowY: "auto", padding: "0.4rem 0" }}>
                              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.3rem 0.75rem", borderBottom: "1px solid #f1f5f9", marginBottom: "0.25rem" }}>
                                ...
                                <div style={{ display: "flex", gap: 6 }}>
                                  <button type="button" style={{ fontSize: "0.7rem", color: "var(--weft-primary,#0078D4)", background: "none", border: "none", cursor: "pointer", padding: 0 }} onClick={() => setHiddenColumns(new Set())}>All</button>
                              ...
                              {results.columns.map((col, idx) => (
                                <label key={idx} style={{ display: "flex", alignItems: "center", gap: 8, padding: "0.3rem 0.75rem", cursor: "pointer", fontSize: "0.8rem", color: "#1e293b" }}
                                  ...
                                  onMouseLeave={(e) => (e.currentTarget.style.background = "")}
                                >
                                  <input type="checkbox" checked={!hiddenColumns.has(idx)} onChange={() => setHiddenColumns((prev) => { const next = new Set(prev); next.has(idx) ? next.delete(idx) : next.add(idx); return next; })} style={{ accentColor: "var(--weft-primary,#0078D4)" }} />
                                  {col}
                                </label>
                              ))}
                            </div>
                ...
                >
                  <div className="save-query-modal-header">
                    <h3 id="lab-error-title" className="save-query-modal-title">
                      {typeof sessionStorage !== "undefined" && sessionStorage.getItem("weft_setup_ok") !== "1"
                  ...
                  <div className="save-query-modal-body">
                    {typeof sessionStorage !== "undefined" && sessionStorage.getItem("weft_setup_ok") !== "1" ? (
                      <p style={{ marginBottom: 8 }}>
                        Weft needs a metadata database to load data sources.{" "}
          ...
          <div className="save-query-modal" role="dialog" aria-modal="true" aria-labelledby="visualize-title">
            ...
            <div className="save-query-modal-body">
              <p style={{ fontSize: "0.85rem", color: "#374151", marginBottom: 12 }}>
                Weft will create a virtual dataset from your SQL query, then open the chart builder.
              </p>
