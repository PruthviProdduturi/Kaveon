...
def _fernet_key() -> bytes:
    secret = settings.AI_ENCRYPTION_SECRET or "weft-default-encryption-secret"
...
def setup_azure_app_roles(graph_token: str, client_id: str, tenant_id: str) -> dict:
    """
    Create the 4 Weft App Roles in the Azure AD App Registration using a
    ...
    # ── 2. Merge: keep existing, append only missing Weft roles ──────────────
    created: list[str] = []
    skipped: list[str] = []
    merged = list(existing_roles)

    for role in WEFT_APP_ROLES:
    ...
    if not created:
        return {
            ...
            "created": [],
            "skipped": skipped,
            "message": "All Weft App Roles already exist in the App Registration.",
        }
