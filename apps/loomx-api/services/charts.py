...
def list_charts(user_id: Optional[str] = None) -> List[dict]:
    if user_id:
        result = db.query("""
            ...
            WHERE c.id IS NOT NULL ORDER BY c.updated_at DESC
        """, [user_id])
    else:
        result = db.query("""
            SELECT c.id, c.name, c.description, c.chart_type, c.query_config, c.viz_config,
                   c.created_on, c.created_by, c.changed_on, c.updated_by, c.created_at, c.updated_at,
                   ds.dataset_name, 0 as favorite
            FROM dbo.charts c
            LEFT JOIN dbo.datasets ds ON ds.id = TRY_CAST(JSON_VALUE(c.query_config, '$.dataset_id') AS INT)
            WHERE c.id IS NOT NULL ORDER BY c.updated_at DESC
        """)
...
def get_chart_by_id(chart_id: str) -> Optional[dict]:
    row = db.query_one("""
