...
|---|---|---|---|---|
| Repository | `dev` at `18cf1de` | Full Rust workspace tests and benchmark targets, strict workspace Clippy/formatting, 77 API tests plus four subtests, and docs validation pass locally | This evidence is not deployed AKS evidence, the Helm chart awaits strict CI rendering, and no external PostgreSQL/Trino measurements were run | Require CI/Engine/Containers, then enable the ADLS store in AKS and run deployment qualification |
