...
2. Current AKS digests are Engine `sha256:bd5a6ef6cdb00a121f215947c98a948f5e3fbd9d2c4b094a7d946d07a71d2bc6`,
...
evidence. The score is capped below 8/10 by three release-critical gaps: the new
exchange cleanup lacks deployed restart evidence, no general relational OLTP semantics,
and no passing publication-scale comparison report. The best
...
| AKS runtime | Engine `bd5a6ef6`; 1 coordinator and 3 workers Ready | 12/12 concurrent known-result queries, forced in-flight worker loss with attempt-1 retry and exact result, 3/3 compatible recovery, 12/12 bounded-pressure queries, no restart/file-count growth, memory below pod limits | Eight pre-existing coordinator exchange chunks survive in the deployed image; local startup reconciliation is implemented but not rolled out, and multi-coordinator consistency is unproved | Deploy startup exchange cleanup and run the combined restart gate, proving stale removal and retained-result preservation |
...
| Distributed analytics | Engine digest `bd5a6ef6` | Three-worker exact-result concurrency and a four-stage join survive forced worker loss through an observed alternate-worker retry | Snapshot mutation during a running query and Trino parity remain unproved | Run the catalog-mutation race against a committed snapshot |
