...
| 2026-09-11 | Claude | The DLM now holds a conversation, still with no model. (1) Ambiguity is a question, not a guess: when two metrics or two dimensions score equally for a question (`revenue by region` against Gross and Net revenue), `ask()` returns `reason: "clarify"` with the tied options and a `resume` payload; Chat renders the options as buttons and re-posts the same question with the slot pinned in `choices`. (2) Scope gate: a question with no token in any dataset's vocabulary (metric, column, dataset names, value index) returns `reason: "out_of_scope"` with the dataset names; a near-miss stays `no_dataset` so the in-browser fallback still runs. (3) Context is client-carried: every answer returns a `frame` (dataset, metric, grouping, filters, year, relative time); Chat sends the last frame with the next question and a follow-up (`what about 2024?`, `filter that by customer Contoso`) inherits every slot it does not restate. Session state stays where it was — in the browser — so the API remains stateless and PostgreSQL gains nothing. (4) Typos resolve by bounded optimal-string-alignment distance against the routed dataset's vocabulary (`net revnue by regoin` → `net_revenue` grouped by `region`), one edit for 4–7-character tokens, two for longer, never touching known words, digits or stopwords. Files: `api/dlm/engine.py`, `api/routers/dlm.py` (`AskBody.choices`, `AskBody.frame`), `api/dlm/test_ask_dialogue.py` (9 tests), `studio/app/page.tsx`. Suite: 249 API tests pass; Chat verified in the browser against a fixture API for all four paths. Embeddings stay out until the query history shows a miss rate the edit-distance and clarification paths do not close. |
