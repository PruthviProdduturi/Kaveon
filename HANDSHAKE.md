...
| 2026-09-11 | Codex | Corrected ADLS Parquet footer reuse so every logical open first rechecks the mutable path with an authoritative `HEAD` and requires an ETag or object version. The bounded 256-entry footer cache and its single-flight locks are keyed by exact account/container/path plus size/timestamp/ETag/version; decoded batches already use the same immutable identity. Replacement tests prove a new footer and row count are selected and an old pinned reader fails its conditional read. All 53 storage tests and strict storage/server Clippy pass. This is local correctness evidence only; no image was built, deployed or benchmarked. |
