...
|---|---|---|---|---|
| Repository | `dev` at `c18debf` | Full Rust workspace tests and benchmark targets, strict workspace Clippy/formatting, 80 API tests plus four subtests, seven comparison-runner tests, Bicep build, and docs validation pass locally | This evidence is not deployed AKS evidence, the Helm chart awaits strict CI rendering, and no external PostgreSQL/Trino measurements were run | Require CI/Engine/Containers, deploy the scoped ADLS resources, then run live qualification |
