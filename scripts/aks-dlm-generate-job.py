...
manifest to stdout; apply it with kubectl.
  ...
  python scripts/aks-dlm-generate-job.py --dataset 15,16,17 --image <repo>@sha256:... | kubectl apply -f -
"""
import argparse
import json
import subprocess
...
for dataset in DATASETS:
    ...
    try:
        ...
        print('RESULT', dataset, json.dumps({'ok': False, 'error': str(getattr(exc, 'detail', exc))[:200]}), round(time.time() - t, 1), 's', flush=True)
        continue
    ...
    print('RESULT', dataset, json.dumps({k: r.get(k) for k in ('ok', 'status', 'rebuilt')}), json.dumps({
...
def main():
    ...
    parser.add_argument("--dataset", required=True, help="dataset id, or a comma-separated list built in order")
    ...
    job = {
        ...
        "metadata": {"name": "kaveon-dlm-generate-" + args.dataset.replace(",", "-"), "namespace": args.namespace,
        ...
        "spec": {"backoffLimit": 0, "ttlSecondsAfterFinished": 86400, "template": {
            ...
            "spec": {
                ...
                "containers": [{
                    ...
                    "command": ["python", "-u", "-c", SCRIPT.replace("DATASETS", json.dumps([d.strip() for d in args.dataset.split(",")]))],
